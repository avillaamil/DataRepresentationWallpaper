...
void setup() {
  ...
  //for ( int i= 0; i < binHash.size(); i++ ){ //binHash.size will tell it how many keys are in the binMap
  for ( ChemicalBin thisCbin : binHash.values()){
    ...
    //confused about which variable to pour into the constructor ... binMap
    //Toxin newToxin = new Toxin( thisPoint.x, thisPoint.y, random( 10, 40 ), thisCbin); //instead of random, it needs to know the value of the total releases
    Toxin newToxin = new Toxin( thisPoint.x, thisPoint.y, thisCbin.aggregateRelease(), thisCbin); //instead of random, it needs to know the value of the total releases
...
void displayToxins(){
  for ( Toxin thisToxin : toxins ){ //this for loop works on collections--- goes through the collection of objects
    thisToxin.display(); 
